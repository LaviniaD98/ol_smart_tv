...
class _SearchPageState extends State<SearchPage> {
          ...
          title: Row(
            children: [
                  ...
                  child: ReactiveForm(
                    ...
                    child: ReactiveValueListenableBuilder(
                        ...
                        return ReactiveSearchInput(
                          formControlName: 'search',
                          autoFocus: true,
                          showReset: form.findControl('search')?.value != null && (form.findControl('search')?.value as String).isNotEmpty,
                          onReset: () {
                            form.reset();
                            context.read<SearchCubit>().localSuggestions();
                          },
                          onChanged: context.read<SearchCubit>().onChanged,
                          onSubmitted: current.valid ? (control) {
                              context.read<SearchCubit>().initPagingController(control.value as String);
              ...
              GestureDetector(
                ...
                child: Text(
                  LabelsManager().getRemoteStringFromLabelKeys(RemoteLabelKeys.undo),
            ...
            child: BlocBuilder<SearchCubit, SearchState>(
              builder: (context, state) => state.maybeWhen(
                initial: (suggestions) => LocalSuggestions(
                  suggestions,
                  onDelete: () => context.read<SearchCubit>().localSuggestions(true),
                  onTap: (control) {
  ...
  Widget _suggestions(List<SuggestionModel> items, String search) => Suggestions(items, search: search);
  ...
  Widget _searchPaginated(BuildContext context) {
    ...
    return PagedListView<int, LearningObjectModel>.separated(
      ...
      builderDelegate: PagedChildBuilderDelegate<LearningObjectModel>(
        itemBuilder: (context, item, index)  => _pagedListItem(item, index, cubit.totalElements),
        ...
        noItemsFoundIndicatorBuilder: (context) => _empty(form.findControl('search')?.value),
  ...
  Widget _error(Failure failure) => Center(
        child: ErrorScreen(
          title: LabelsManager().getRemoteStringFromLabelKeys(RemoteLabelKeys.error),
          message: failure.error ?? LabelsManager().getRemoteStringFromLabelKeys(RemoteLabelKeys.error_occurred),
        ),
      );
  ...
  Widget _empty(String? value) => Center(
        child: Padding(
          ...
          child: Text(
            value != null ? LabelsManager().getRemoteStringFromLabelKeys(RemoteLabelKeys.search_empty_text).replaceFirst('{{value}}', value) : '',
  ...
  Widget _pagedListItem(LearningObjectModel item, int index, int? totalElements) {
      ...
      return Column(
        ...
        children: [
          Padding(
            ...
            child: Text(
              LabelsManager().getRemoteStringFromLabelKeys(RemoteLabelKeys.search_count_result).replaceFirst('{{count}}', '$totalElements'),
              textAlign: TextAlign.center,
              style: AppTextTheme.body(
                color: ColorManager().getColorTextPrimary(),
              ),
