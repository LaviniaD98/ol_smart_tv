...
String formatReadingTime(String? body, AttachmentsModel? attachments) {
  ...
  return switch (totalLength) {
    < 60 => //Seconds
    LabelsManager().getRemoteStringFromLabelKeys(RemoteLabelKeys.sec)
        .replaceFirst('{{count}}', '$totalLength'),
    < (60*60) => //Minutes
    "${(totalLength / 60.0).round()} ${LabelsManager().getRemoteStringFromLabelKeys(RemoteLabelKeys.min)}",
    < (60*60)*2 => //Hour
    LabelsManager().getRemoteStringFromLabelKeys(RemoteLabelKeys.hour_one)
        .replaceFirst('{{count}}', '1'),
    _ => //Hours
    LabelsManager().getRemoteStringFromLabelKeys(RemoteLabelKeys.hour_other)
        .replaceFirst('{{count}}', '${(totalLength / (60.0 * 60.0)).round()}'),
  };
}